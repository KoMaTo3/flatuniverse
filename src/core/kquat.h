...
KM_INLINE CQuat::CQuat( void ) {
}

KM_INLINE CQuat::CQuat( float x, float y, float z ) {
	this->x = x;
	this->y = y;
	this->z = z;
}

KM_INLINE void CQuat::Set( float x, float y, float z ) {
	this->x = x;
	this->y = y;
	this->z = z;
}

KM_INLINE float CQuat::operator[]( int index ) const {
	assert( ( index >= 0 ) && ( index < 3 ) );
	return ( &x )[ index ];
}

KM_INLINE float& CQuat::operator[]( int index ) {
	assert( ( index >= 0 ) && ( index < 3 ) );
	return ( &x )[ index ];
}

KM_INLINE bool CQuat::Compare( const CQuat &a ) const {
	return ( ( x == a.x ) && ( y == a.y ) && ( z == a.z ) );
}

KM_INLINE bool CQuat::Compare( const CQuat &a, const float epsilon ) const {
	if ( Math::Fabs( x - a.x ) > epsilon ) {
		return false;
	}
	if ( Math::Fabs( y - a.y ) > epsilon ) {
...
KM_INLINE bool CQuat::operator==( const CQuat &a ) const {
	return Compare( a );
}
...
KM_INLINE int CQuat::GetDimension( void ) const {
	return 3;
}

KM_INLINE Quat CQuat::ToQuat( void ) const {
	// take the absolute value because floating point rounding may cause the dot of x,y,z to be larger than 1
	return Quat( x, y, z, sqrt( fabs( 1.0f - ( x * x + y * y + z * z ) ) ) );
}

KM_INLINE const float *CQuat::ToFloatPtr( void ) const {
	return &x;
}

KM_INLINE float *CQuat::ToFloatPtr( void ) {
	return &x;
}

extern const Quat QuatNull;
